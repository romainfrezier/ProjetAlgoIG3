...
func choixModeCalcul() -> MethodeCalcul {
    ...
    if let modeCalcul = readInt() {
        if modeCalcul == 1 {
            return MethodeCalcul.plusGrandGroupe
        } else if modeCalcul == 2 {
            return MethodeCalcul.multiplicationGroupes
        }
    }
...
// retourne un tableau avec 4 coordonnées :
...
func choixCoordonneesDeplacement() -> [Int] {
    ...
    if let xDebut = readInt(), let yDebut = readInt() {
        print("Jusqu'à quelle case ? (veuillez saisir la valeur de x, appuyez sur Entrée puis saisissez la valeur de y)")
        if let xFin = readInt(), let yFin = readInt() {
