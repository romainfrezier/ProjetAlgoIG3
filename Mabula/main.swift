...
while !jeu.partieFinie() {
    print("C’est au tour du joueur ", jeu.joueurActuel, ". (blanc: o, noir: x)", separator: "")
    ...
    let caseJouee = jeu.recupererContenuCase(x: xDebut, y: yDebut)
    if let caseJouee = caseJouee{
      if(caseJouee.estBlanc && jeu.joueurActuel == Joueur.blanc){
      ...
      else if (caseJouee.estNoir && jeu.joueurActuel == Joueur.noir){
        jeu.deplacerBille(xDebut: xDebut, yDebut: yDebut, nbCasesDeplacement: nbCasesDeplacement)
        // on récupère le joueur actuel
        // si c'est nil on ne fait rien car la partie est finie
        if let joueur = jeu.prochainJoueur(){
        ...
        print("vous ne pouvez pas jouer une bille qui ne vous appartient pas")
      }
    }
}
